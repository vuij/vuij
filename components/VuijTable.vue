    ...
    <table ref="tableRef" class="vuij-table">
        ...
        <tbody>
            ...
            <template v-for="(row, rowIx) in rows" :key="row.id">
                <!-- Родительская строка -->
                <tr :class="['vuij-table__row', { expanded: expandedRows.has(row.id) }, rowClass(row)]" @click="handleRowClick(row)">
...
const emit = defineEmits(['row-click', 'select-change', 'sort-change'])
