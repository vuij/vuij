<template>
    <VuijInputPop 
        ...
        v-model="searchValue" 
        :readonly="!filterable" 
        :data-placeholder="!filterable?'...':$attrs.placeholder"
        v-bind="$attrs" 
        :opened="isOpen" 
    ...
    >
        ...
        <template #stretch>
            ...
            <template v-if="tagged">
                <VuijTags :tags="selectedOptions" :skimp="skimp" :normalize="false" @click.self="handleTagsClickSelf"  @close="(e: Event, tag: OptionItem) => handleTagClose(tag)">
            ...
            <div class="vuij-card vuij-card_mystic vuij-card_options">
                <VuijLoading>
                    <template v-if="!loading">
                        ...
                        >
                            <!-- slot nooption -->
                            <template #default="optionSlotProps">
                                <slot name="option" v-bind="optionSlotProps">{{ optionSlotProps.item.label }}</slot>
                            </template>
                        </VuijOptions>
                    </template>
                </VuijLoading>
...
import VuijInputPop from '@/vuij/components/VuijInputPop.vue';
...
const normalizer = (item: any, labelKey: string, valueKey: string|number) => (typeof item === 'object' && item !== null) // import utils
    ? { label: item[labelKey]?.toString() || '', value: item[valueKey] } 
