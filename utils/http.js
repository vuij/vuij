...
class Http {
    ...
    static _findOrFirst(needle, haystack) {
        ...
        return haystack.includes(needle) ?? haystack[0];
    }
    ...
    correctMethod() {
        if(!this.method) this.method = this.getMethod(this.method);

        if(this.method === 'GET' || this.method === 'POST') return;
        else if(this.params?.jsonRequest===true) return;
        else {
            // LARAVEL way
    ...
    correctHeaders() {
        if(!this.params.STRICT) {
            if(!!this.params.apiToken) { // && typeof 'string'
                if(this.params.jsonRequest===undefined) this.params.jsonRequest = true;
    ...
    setOptions(options, _options, _params) {
        ...
        this.correctParamsOptions();

        this.setCorrectBody(options.body);
    }

    correctParamsOptions() {
        this.correctHeaders();
        this.correctMethod();
        ...
        */
        ...
        if(!_body) _body = undefined;
            ...
            if(this.params.STRICT) {
                ...
                // LARAVEL way
                if(!this.params.jsonRequest && this._method) {
                    _body.append('_method', this._method);
                }
    ...
    async fetch(url, options) {
        ...
        this.setOptions(options, this.options, this.params)
